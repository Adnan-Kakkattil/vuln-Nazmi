 ...
 * 
 * POST /api/v1/integration/token.php
...
try {
    ...
    if (!isValidApiKeyFormat($apiKey)) {
        logApiRequest($pdo, null, $apiKey, 'auth', 'POST', '/api/v1/integration/token', 
    ...
    if (!isIpAllowed($keyData['allowed_ips'], $_SERVER['REMOTE_ADDR'] ?? '')) {
        logApiRequest($pdo, $keyData['id'], $apiKey, 'auth', 'POST', '/api/v1/integration/token', 
                     $_SERVER['REMOTE_ADDR'] ?? null, $_SERVER['HTTP_USER_AGENT'] ?? null, 
    ...
    logApiRequest($pdo, null, $apiKey ?? null, 'auth', 'POST', '/api/v1/integration/token', 
                 $_SERVER['REMOTE_ADDR'] ?? null, $_SERVER['HTTP_USER_AGENT'] ?? null, 
